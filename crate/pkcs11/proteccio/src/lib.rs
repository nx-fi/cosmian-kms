...
//         cryptoki_fn!(fn $name($($arg: $type),*) {Err(PError::FunctionNotSupported)});
//     };
// }
//
// #[macro_export]
...
//     ($ptr:expr) => {
//         if $ptr.is_null() {
//             return Err(PError::ArgumentsBad);
//         }
//     };
// }
//
...
//             return Err(PError::CryptokiNotInitialized);
//         }
//     };
// }
//
...
//             return Err(PError::SessionHandleInvalid($handle));
//         }
//     };
// }
//
...
//     ($id:expr) => {
//         if $id != SLOT_ID {
//             return Err(PError::SlotIdInvalid($id));
//         }
//     };
// }
...
//             return Err(PError::CryptokiAlreadyInitialized);
//         }
//         Ok(())
//     }
// );
...
//                 return Err(PError::BufferTooSmall);
//             }
...
//             return Err(PError::MechanismInvalid(mechType));
//         }
...
//         initialized!();
//         valid_slot!(slotID);
//         Err(PError::TokenWriteProtected)
//     }
// );
//
// cryptoki_fn!(
...
//             return Err(PError::SessionParallelNotSupported);
//         }
...
//             return Ok(());
//         }
//         Err(PError::SessionHandleInvalid(hSession))
//     }
// );
//
// cryptoki_fn!(
...
//         not_null!(pTemplate);
//
//         sessions::session(hSession, |session| -> PResult<()> {
//             let find_ctx = OBJECTS_STORE
//                 .read()
//                 .map_err(|_| PError::OperationNotInitialized(hSession))?;
//             let object = match find_ctx.get_using_handle(hObject) {
//                 Some(object) => object,
//                 None => {
//                     return Err(PError::ObjectHandleInvalid(hObject));
//                 }
//             };
...
//                     .type_
//                     .try_into()
//                     .map_err(|_| PError::AttributeTypeInvalid(attribute.type_))?;
//                 info!(
...
//                     Ok(())
//                 }
//                 Some(_) | None => Err(PError::KeyHandleInvalid(hKey)),
//             }
//         })
//     }
// );
...
//         not_null!(pPart);
//         not_null!(pulPartLen);
//         Err(PError::FunctionNotSupported)
//     }
// );
//
// cryptoki_fn!(
...
//                 Some(_) | None => return Err(PError::KeyHandleInvalid(hKey)),
//             };
//             let mechanism = unsafe { parse_mechanism(pMechanism.read()) }?;
...
//                 None => return Err(PError::OperationNotInitialized(hSession)),
...
//         initialized!();
//         valid_session!(hSession);
//         Err(PError::FunctionNotParallel)
//     }
// );
//
// cryptoki_fn!(

...
use crate::database::{unwrapped_cache::UnwrappedCache, Database};

pub(crate) struct Store {
    /// A map of uid prefixes to Objects Database
    /// The "no-prefix" DB is registered under the empty string
    objects: RwLock<HashMap<String, Arc<dyn Database + Sync + Send>>>,
    ...
    permissions: Arc<dyn Database + Sync + Send>,
}
...
impl Store {
    ...
    pub(crate) fn new(
        default_objects_database: Arc<dyn Database + Sync + Send>,
        permissions_database: Arc<dyn Database + Sync + Send>,
    ) -> Self {
        Self {

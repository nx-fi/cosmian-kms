...
echo "Setup for OpenSSL version $OPENSSL_VERSION without FIPS module"
...
if [ "${2}" = "cross-compile-windows" ]; then
    ./Configure mingw64 --cross-compile-prefix=x86_64-w64-mingw32- --prefix="${OPENSSL_DIR}" --openssldir="${OPENSSL_DIR}/ssl" threads no-shared enable-weak-ssl-ciphers
else
    ./Configure --prefix="${OPENSSL_DIR}" --openssldir="${OPENSSL_DIR}/ssl" threads no-shared enable-weak-ssl-ciphers
fi
